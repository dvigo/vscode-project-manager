...
suite('Extension Test Suite', () => {
	...
	test('contributes expected commands', async () => {
		const extension = vscode.extensions.all.find((item) => item.packageJSON.name === 'simple-workspace-manager');
		assert.ok(extension, 'Development extension simple-workspace-manager was not found.');
